...
const SortableList: React.FC< SortableListProps > = ( {
	...
	const preparedItems = useSelect(
		( select ) => {
			...
			return posts.reduce< { [ key: string ]: PickedItemType | null } >(
				( acc, item ) => {
					...
					const result = getEntityRecord< Post | Term | User >(
						...getEntityRecordParameters
					);
		...
		( event: DragEndEvent ) => {
			...
			if ( active.id !== over?.id ) {
				const oldIndex = posts.findIndex(
					( post ) => post.uuid === active.id
				);
				const newIndex = posts.findIndex(
					( post ) => post.uuid === over?.id
				);
	...
	return (
		...
		>
			...
			<DragOverlay dropAnimation={ dropAnimation }>
				{ activeId && activePost ? (
					<DraggableChip title={ activePost.title } />
				) : null }
			</DragOverlay>
		</DndContext>
	);
